...
def compute_scores(pop_k, pop_p, string, dict_bigrams, indexes, cores=1):
    ...
    """
    Returns two lists with computed fitnesses for a population of keys with different
...
#A pretty good alternative is the comparing only on the fitness_key part
def is_vigenere(fitness_key, fitness_perm, lim_key=1500, lim_perm=89.5):
    ...
    if max(fitness_key) > lim_key and fitness_perm < lim_perm:
        ...
        print("WARNING: cannot decide if Vigenere or permutation!\nRe-run increasing number of generations\n")
...
def main():
    ...
    #if None, set the same values as subpopulation
    if npop_perm == None:
        npop_perm = npop
    ...
    values = [npop, npop_perm, ngen, p_elite, p_xover, mut_prob, mut_prob_perm] + [
             "None" if seed == None else seed] + [klmin, klmax-1, cores, filename]

    names = ["Population size per key", "Population size (permutation)", "Generations",
             "Fraction of elite", "Crossover probability", "Mutation probability",
             "Mutation probability (permutation)", "Random seed", "Min key length",
             "Max key length", "Cores", "Encrypted text in file"]
    ...
    #instanciate lists
    pop_key, pop_perm = [], []                       #populations
    scores_key, scores_perm = [], []                 #fitnesses
    max_list, ave_list, max_list_perm = [], [], []   #max and ave fitness at each step
    ...
    for step in range(ngen):
        ...
        for i in range(nkeys):
            new_pop_key.extend(ga.generate_new_population(pop_key[index[i] : index[i+1]], text, int(p_elite*npop),
    ...
    #find if polyalphabetic or monoalphabetic cipher and print to file
